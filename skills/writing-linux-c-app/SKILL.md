...
description: Use this skill whenever the user works on Linux userspace C: planning or building apps and tools, CLI utilities, daemons, services, CMake projects, build.sh, install/deploy under build/ and deploy/, cross-compilation, logging and config conventions, or adopting the bundled small-app vs complex-app template flow—even if they only want a roadmap, design doc, task breakdown, rough estimate, or say "a C program on Linux" without naming CMake or this skill. Apply during planning and design, not only after implementation. Before a detailed plan or any repo change: (1) explicit consent to follow this Skill; (2) a class choice 小型 / 复杂 / 其他, or one-line confirmation when the user already stated an equivalent. Excludes kernel/driver work and workflows that are not CMake-first.
---
